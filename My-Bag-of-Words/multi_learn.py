...
import argparse
from gen_sets import gen_tt_sets
...
if __name__ == '__main__':
    print "---------------------"
    print "## loading images and generating training sets"
    args = parse_arguments()
    datasetpath = args.d
    gen_tt_sets(NUM_SETS, datasetpath)
    ...
    for set in range(0, NUM_SETS):
        ...
        for cat_path, label in zip(files.keys(), range(len(cats))):
            ...
            cat_label[basename(cat_path)] = label
            for j in cat_files:
                all_files_labels[j] = label
        ...
        with open(datasetpath + str(set) + '.' + CODEBOOK_FILE, 'wb') as f:
        ...
        print "---------------------"
        print "## train svm"
        c, g, rate, model_file = libsvm.grid(datasetpath + str(set) + '.' + HISTOGRAMS_FILE,
                                         png_filename='grid_res_img_file.png')

        print "--------------------"
        print "## outputting results"
        print "model file: " + model_file 
        print "codebook file: " + datasetpath + str(set) + '.' + CODEBOOK_FILE
