...
// taken from feng's work.
 ...
 void Clustering::buildDataFromMoFREAK(std::deque<MoFREAKFeature> mofreak_ftrs, bool sample_pts, bool img_diff, bool fix_class, int fixed_class)
 {
	 ...
	 for (unsigned int row = 0; row < num_ftrs; ++row)
	 {
        ...
        data_pts->at<unsigned char>(row, 1) = (unsigned char)ftr.game;
 ...
 void Clustering::writeClusters(int group, bool append)
 {
	 cout << "writing" << endl;
	 std::ofstream output_file;

     stringstream cluster_filepath;
     cluster_filepath << SVM_PATH << "/clusters" << group << ".txt";

	 if (append)
	 {
         output_file.open(cluster_filepath.str(), std::ios_base::app);
	 }
	 else
	 {
