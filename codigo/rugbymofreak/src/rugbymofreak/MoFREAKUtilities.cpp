...

{
	...
	while (true)
	{
        ...
        //cv::BriskFeatureDetector *detector = new cv::BriskFeatureDetector(30); //unused
		...
		for (auto keypt = keypoints.begin(); keypt != keypoints.end(); ++keypt)
		{
			...
			if (sufficientMotion(current_frame, prev_frame, keypt->pt.x, keypt->pt.y, keypt->size))
			{
				...
				// gather metadata.
                int action, game, video_number;
                readMetadata(video_filename, action, video_number, game);
...
void MoFREAKUtilities::readMoFREAKFeatures(std::string filename, int num_to_sample)
{
    ...
    int action, video_number, game;
    readMetadata(filename, action, video_number, game);
